  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-lg">
      <div className="bg-card border border-card-border rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        <div className="bg-card-sub-bg/50 px-6 py-5">
        ...
        <div className="p-6">
              ...
              return (
                ...
                >
                  <div className={`relative flex items-center justify-center w-10 h-10 rounded-full shrink-0 transition-all duration-300 
                    ${isCompleted ? "bg-green-500 text-white" : isCurrent ? "bg-primary text-white ring-2 ring-indigo-400 ring-offset-2 ring-offset-background" : "bg-card-sub-bg text-text-muted"}`}>
                    {isCompleted ? (
                  ...
                  <div className="flex-1 pt-1">
                    <p className={`font-medium text-sm ${isCurrent ? 'text-text-primary font-bold' : isCompleted ? 'text-text-secondary' : 'text-text-muted'}`}>
                      {step.label}
                      {isCompleted && <span className="ml-2 text-xs font-normal bg-green-600/50 text-green-300 px-1.5 py-0.5 rounded-sm">완료</span>}
                    </p>
                    {isCurrent && <p className="text-xs text-text-secondary mt-0.5">{step.description}</p>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="mt-6 pt-4 border-t border-card-border">
            ...
            <div className="mt-2 h-2 bg-card-sub-bg rounded-full overflow-hidden relative">
              <div className="h-full bg-gradient-to-r from-primary/50 via-primary to-primary/50 animate-progress-shine rounded-full transition-all duration-500"
