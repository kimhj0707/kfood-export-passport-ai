...
- **US가 더 완성된 이유**: 키워드 풍부도 차이 (US: 8개 알레르기, 다양한 변형)
...
5. **결과 페이지**에서 OCR 텍스트, 알레르기, 규정 검토, 홍보 문구 확인
6. "PDF 다운로드" 클릭
...
└── kfood-export-passport-ai-frontend/  # 별도 git 레포
```
...
- [x] `backend/` 폴더 생성 및 백엔드 파일 이동
- [x] `frontend/` 폴더로 프론트엔드 이동
- [x] `docs/` 폴더 생성 및 문서 이동
- [x] 루트 `.gitignore` 업데이트 (env.yaml, credentials 등 추가)
- [x] GitHub 모노레포 생성 및 푸시
- [x] Vercel 새 프로젝트 설정 (Root Directory: frontend/)
...
| 서비스                     | URL                                                         |
| -------------------------- | ----------------------------------------------------------- |
| **GitHub (모노레포)**      | https://github.com/kimhj0707/kfood-export-passport-ai       |
| **프론트엔드 (Vercel)**    | https://frontend-liart-five-92.vercel.app                   |
| **백엔드 API (Cloud Run)** | https://kfood-api-233469550454.asia-northeast3.run.app      |
...
| 기능                  | 설명                | 상태    |
| --------------------- | ------------------- | ------- |
| 히스토리 페이지네이션 | 이전/다음 버튼 동작 | 🔄 예정 |
| 이미지 미리보기       | 업로드 이미지 확인  | 🔄 예정 |
| 리포트 삭제 버튼      | UI에서 삭제 기능    | 🔄 예정 |
...
**Cloud Run**: 환경 변수로 OPENAI_API_KEY 설정
